...
<template>
  <HoverTilt
    ...
    <div class="flex flex-col md:flex-row grow ">
      <div
        class="overflow-hidden rounded-md drop-shadow-[0_0_20px_rgba(16,152,173,0.8)] md:w-5/12 md:mt-10 mx-2 h-3/5 md:h-auto">
        <img src="/images/icon.png" alt="logo" class="object-cover w-full h-full">
      </div>
      <div class="flex flex-col gap-2 md:w-7/12 md:my-auto h-2/5 md:h-auto">
        ...
        <p class="h-10 text-white text-md mx-auto">
          <span class="line-through">Fullstack</span> Web Engineer
        </p>
        <div class="flex flex-col row-span-2 gap-2 items-left mx-auto">
          <NuxtLink
            v-for="link in Object.values(links)" :key="link.name" :to="link.url"
            target="_blank" rel="noopener noreferrer" class="flex items-center gap-1 group">
            <span class="flex items-center justify-center transition-all duration-300 rounded-full bg-teal-700/50 size-8 group-hover:bg-teal-700/70 group-hover:scale-110 group-active:scale-95">
              <Icon :name="link.icon" />
            </span>
            <span class="text-xs transition-all duration-300 text-gray-400 font-bold group-hover:scale-105 origin-top-left">{{ link.url }}</span>
          </NuxtLink>
        </div>
      </div>
    </div>
