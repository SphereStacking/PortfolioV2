...
const htmlPreview = computed(() => {
  const lines = generateMarkdown.value.trim().split('\n')
  ...
  // ボディ
  html += '<tbody>'
  const startIndex = hasHeader.value ? 2 : 0
  for (let i = startIndex; i < lines.length; i++) {
    if (i === 1) continue // 区切り行をスキップ
    html += '<tr>'
...
<template>
  <div class="space-y-6">
    ...
    <Card>
      ...
      <CardContent>
        <div v-html="htmlPreview" class="markdown-preview"></div>
      </CardContent>
    </Card>
  </div>
</template>
